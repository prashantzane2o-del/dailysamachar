        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={onClose} aria-label="Close dialog" className="hover:bg-soft rounded-lg p-2">
                <X size={18} />
              </button>
            </div>
            {children}
      ...
      {open && (
        <>
          ...
          <motion.aside
            ...
            className="bg-paper fixed top-0 right-0 z-50 h-full w-full max-w-sm p-6 shadow-2xl"
          >
            {children}
          </motion.aside>
        </>
    ...
    <div className="divide-y border-y">
      {items.map((item, index) => (
        <div key={item.title}>
          ...
          >
            {item.title}
            <ChevronDown className={cn("transition", active === index && "rotate-180")} size={17} />
          </button>
          <AnimatePresence>
            {active === index && (
              <motion.p
    ...
    <div role="tablist" className="flex gap-1 overflow-x-auto border-b">
      {tabs.map((tab) => (
        <button
          ...
          className={cn(
            "shrink-0 border-b-2 px-3 py-2 text-xs font-bold",
            active === tab ? "border-signal text-signal" : "text-muted border-transparent",
          )}
        >
          {tab}
        </button>
      ...
      {Array.from({ length: total }, (_, i) => (
        <button
          ...
          className={cn(
            ...
            current === i + 1 ? "bg-ink text-white" : "hover:bg-soft",
          )}
        >
          {i + 1}
        </button>
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 20 }}
          className="bg-ink fixed bottom-5 left-1/2 z-70 -translate-x-1/2 rounded-xl px-4 py-3 text-sm font-semibold text-white shadow-2xl"
        >
          {message}
        </motion.div>
      )}
...
export function Skeleton({ className }: { className?: string }) {
  return <div className={cn("rounded-lg bg-slate-200 motion-safe:animate-pulse", className)} />;
}
