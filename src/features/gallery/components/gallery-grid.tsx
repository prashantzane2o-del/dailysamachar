    ...
    <>
      <div className="columns-1 gap-4 sm:columns-2 lg:columns-3">
        {articles.map((article) => (
          ...
          >
            <Image src={article.image} alt="" width={800} height={560} className="h-auto w-full object-cover" />
      ...
      <Modal open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.title ?? "Gallery image"}>
        {selected && <Image src={selected.image} alt="" width={1200} height={800} className="w-full rounded-xl" />}
      </Modal>
    </>
  );
}
