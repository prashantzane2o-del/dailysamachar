  ...
  return (
    ...
    >
      <div className="mb-6 flex items-center gap-3 border-b border-line pb-4">
        ...
        <h2 className="text-xl font-bold text-ink">BMI Calculator</h2>
      </div>
      ...
      <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label htmlFor="bmi-height" className="text-sm font-semibold text-muted">
              Height (cm)
            </label>
          ...
          <input
            ...
            value={height}
            onChange={(e) => setHeight(Number(e.target.value))}
            className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-line accent-blue-600 dark:accent-blue-400"
            aria-hidden="true"
          />
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label htmlFor="bmi-weight" className="text-sm font-semibold text-muted">
              Weight (kg)
            </label>
            <div className="relative w-32">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-muted">
      ...
      >
        <div className="flex items-end justify-between">
          <div>
            <p className="text-sm text-muted">Your BMI Score</p>
            <p className={`mt-1 text-3xl font-black ${colorClass}`}>
              {bmi > 0 ? bmi : '--'}
            </p>
          </div>
          <div className="text-right">
            <span className={`inline-block rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${colorClass} bg-paper shadow-sm`}>
              {category}
            </span>
          </div>
        </div>
        ...
        {bmi > 0 && (
          <div className="mt-6">
            ...
            <div 
              ...
              aria-hidden="true"
            />
            <div className="mt-2 flex justify-between text-[10px] uppercase text-muted">
              <span>Under</span>
              <span>Normal</span>
              <span>Over</span>
              <span>Obese</span>
